...
public class AccountService : EntityService<Account>, IAccountService
{
    ...
    public async Task<List<DoctorDto>> GetDoctorsOrg(Place place, Guid doctorID)
    {
        ...
        if (place == Place.INCLUSIVE)
        {
            ...
            .Where(x => x.combined.ar.RoleId != (int)RoleType.PATIENT && Guid.Equals(x.combined.dep.OrganizationId, organizationId) && x.combined.acc.IsDisable == false)
            .GroupBy(x => new
            {
                x.combined.acc.Id,
    ...
    public async Task<List<PatientDto>> GetScheduledPatient(AppointmentStatus status, Guid doctorID)
    {
        try
        {
            ...
            .Where(a => a.Patient.IsDisable == false
                && a.Status == status
                && Guid.Equals(a.DoctorId, doctorID)
    ...
    public async Task<DataDefaultDto> GetDataDefault(Guid accountID)
    {
        try
        {
            var result = await _unitOfWork.AppointmentRepository.GetAll()
                ...
                .Include(a => a.Patient)
                .Include(a => a.Doctor)
                .Where(a => Guid.Equals(a.Id, accountID))
    ...
    {
        var account = await _unitOfWork.AccountRepository.FirstOrDefaultAsync(x => x.StakeId == stakeId) ?? throw new AppException("Account not found!");
